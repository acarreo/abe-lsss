...
TEST(PK_ODPH, TestCCASecurityForScheme) {
  ...
  getRandomBytes(bytes, TEST_MSG_LEN);
  ...
  ASSERT_TRUE(schemeContext->generateParams() == OpenABE_NOERROR);
  ...
  ASSERT_TRUE(schemeContext->encrypt("public_B", "public_A", plaintext1, ciphertext) == OpenABE_NOERROR);
