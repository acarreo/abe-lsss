...
OpenABE_ERROR
OpenABEContextCCA::generateGlobalParams(const string &gpkID) {
  return this->abeSchemeContext->generateGlobalParams(gpkID);
}
...
OpenABE_ERROR
OpenABEContextGenericCCA::generateParams(const string &mpkID, const string &mskID) {
  return this->abeSchemeContext->generateParams(mpkID, mskID);
}
...
OpenABE_ERROR
OpenABEContextGenericCCA::encryptKEM(const string &mpkID, const OpenABEFunctionInput *encryptInput,
  ...
  try {
    ...
    // hashU = H_2(u)
    nonceU = this->getPairing()->hashFromBytes(u, OpenABE_NONCELEN, CCA_HASH_FUNCTION_TWO);

    // compute ciphertext, C
    result = this->abeSchemeContext->encrypt(mpkID, encryptInput, M, ciphertext);
...
OpenABEContextGenericCCA::decryptKEM(const string &mpkID, const string &keyID,
  ...
  try {
    ...
    // compute ciphertext, C
    result = this->abeSchemeContext->encrypt(mpkID, encryptInput.get(), M, *ciphertext2);
    // verification check
 ...
 */
OpenABE_ERROR
OpenABEContextSchemeCCA::generateParams(const string &mpkID, const string &mskID) {
  return this->m_KEM_->generateParams(mpkID, mskID);
}
 ...
 */
OpenABE_ERROR
OpenABEContextSchemeCCA::generateGlobalParams(const string &gpkID) {
  return this->m_KEM_->generateGlobalParams(gpkID);
}
...
OpenABEContextSchemeCCA::encrypt(const string &mpkID,
  ...
  try {
    ...
    result =
        this->m_KEM_->encryptKEM(mpkID, encryptInput,
    ...
    unique_ptr<OpenABESymKeyAuthEnc> authEnc(new OpenABESymKeyAuthEnc(DEFAULT_AES_SEC_LEVEL, symkeyBytes));
    ...
    ciphertext2.setHeader(OpenABE_SCHEME_AES_GCM, ciphertext1.getUID());
...
OpenABEContextSchemeCCA::decrypt(const string &mpkID, const string &keyID,
  ...
  try {
    ...
    symkeyBytes = symkey->getKeyBytes();
    authEnc.reset(
        new OpenABESymKeyAuthEnc(DEFAULT_AES_SEC_LEVEL, symkeyBytes));
    // embed the header of the ciphertext as AAD
    authEnc->setAddAuthData(ctHdr);
 ...
 */
OpenABE_ERROR
OpenABEContextSchemeCCAWithATZN::generateParams(const string &mpkID, const string &mskID) {
  return this->m_KEM_->generateParams(mpkID, mskID);
}
 ...
 */
OpenABE_ERROR
OpenABEContextSchemeCCAWithATZN::generateGlobalParams(const string &gpkID) {
  return this->m_KEM_->generateGlobalParams(gpkID);
}
...
OpenABEContextSchemeCCAWithATZN::encrypt(const string &mpkID,
  ...
  try {
    ...
    result =
        this->m_KEM_->encryptKEM(mpkID, encryptInput, DEFAULT_SYM_KEY_BYTES, symkey, ciphertext);
    ASSERT(result == OpenABE_NOERROR, result);
    ...
    ASSERT_NOTNULL(k);
    ctHash = ComputeHMAC(*k, ctBlob);
    // (3) create the key handle (from key and hash of ABE ciphertext)
    keyHandle.reset(new OpenABESymKeyHandle(symkeyBytes, ctHash)); // no b64 encoding by default
  } catch (OpenABE_ERROR &error) {
